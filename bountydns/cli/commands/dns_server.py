...
class DnsServer(BaseCommand):
    ...
    def parser(cls, parser):
        parser.add_argument('-z', '--zone', action='append', help='zones to answer (only domain:ip)')
        parser.add_argument('-Z', '--zone-file', action='store', help='file (txt) with zones to answer (only domain:ip)')
        parser.add_argument('-u', '--upstream', action='store', help='upstream dns server')
        parser.add_argument('-a', '--api-url', action='store', help='callback api url')
        parser.add_argument('-t', '--api-token', action='store', help='callback api token')
        parser.add_argument('-p', '--port', action='store', type=int, default=53, help='listen port')
        parser.add_argument('-l', '--listen', action='store', default='127.0.0.1', help='bind address')
        return parser
    ...
    def run(self):
        ...
        tcp_server = DNSServer(resolver, address=listen, port=port, tcp=True, logger=DnsLogger())

        logger.info('starting DNS server on port %d, upstream DNS server "%s", %d zones', port, upstream, len(zones))
    ...
    def get_zones(self):
        ...
        if os.getenv('ZONES', None):
            zones = zones + [(zp.split(':')[0], zp.split(':')[1]) for zp in os.getenv('ZONES').split(',')]
        return zones
